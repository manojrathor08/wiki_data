...
def append_dataset(data):
  #Check if the dataset file already exists
    if os.path.isfile(file_path):
        ...
        combined_title = dataset['title']+ data['title']
...
@app.get("/wiki/")
def get_and_save_wikipedia_summary(title:str = Query(...,description='wikipedia_title'),length:int =Query(10, description='Summary length')):
...
def retrieve_datasets():
    try:
        ...
        with open(file_path,'r') as json_file:
            saved_dataset = json.load(json_file)
    except FileNotFoundError:
